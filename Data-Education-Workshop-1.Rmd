...
# Filter finaldata for October 18, 2020 (3 weeks after the start of the war) and join with world map data (choose a date!)
...
#Visualization (bar and age scatter plot) 
...
#Linear relationship plot scatterplot 
#SHOW WITH ONLY ONE VARIABLE ON SCATTER PLOT AND SAME SINGLE VAR MODEL 

#Set Up Train Set and Test Set
train_set_2 <- data[1:814,]
test_set_0_2 <- data[814:1629,] %>% filter(!is.na(sbp)) %>%filter(!is.na(age)) %>% filter(!is.na(sex)) %>% filter(!is.na(race)) %>% filter(!is.na(wt71)) %>% filter(!is.na(smokeintensity)) %>% filter(!is.na(cholesterol)) %>% filter(!is.na(active))
test_set_2 <- test_set_0_2 %>% select(-sbp)

linear_model <- lm(sbp ~ age + sex + race + wt71 + smokeintensity + cholesterol + as.factor(active), data = train_set_2)
...
test_set_0_2.1 <- data[814:1629,] %>% filter(!is.na(sbp)) %>%filter(!is.na(age)) %>% filter(!is.na(sex)) %>% filter(!is.na(race)) %>% filter(!is.na(wt71)) %>% filter(!is.na(smokeintensity)) %>% filter(!is.na(cholesterol)) %>% filter(!is.na(active)) %>% filter(!is.na(hbpmed)) %>% filter(!is.na(hbp)) %>% filter(!is.na(alcoholfreq))
test_set_0_2.1 <- test_set_0_2.1 %>% filter(hbpmed !=2) %>% filter(hbp !=2) %>% filter(alcoholfreq != 5)
test_set_2.1 <- test_set_0_2.1 %>% select(-sbp)

linear_model_2 <- lm(sbp ~ age + sex + race + wt71 + smokeintensity + cholesterol + as.factor(active) + as.factor(hbp) + as.factor(hbpmed) + as.factor(alcoholfreq), data = train_set_2.1)
