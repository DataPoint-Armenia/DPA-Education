...
library(forecast) # Forecast / Predictive Modeling
...
*Data Source:* We will be using “nCOV2019”, an R package developed by Dr. Guangchuang Yu from Southern Medical University and accessible via his public [GitHub](https://github.com/YuLab-SMU/nCov2019). 
...
**Citation**: Tianzhi Wu, Erqiang Hu, Xijin Ge, Guangchuang Yu. [Open-source analytics tools for studying the COVID-19 coronavirus outbreak](https://www.medrxiv.org/content/10.1101/2020.02.25.20027433v2). medRxiv, 2020.02.25.20027433. [doi:](https://doi.org/10.1101/2020.02.25.20027433)

ATTEMPT #1
```{r}
#https://towardsdatascience.com/visualize-the-pandemic-with-r-covid-19-c3443de3b4e4 
#Link of Example I'm Following ^ 

#Loading data from GitHub using the provided information
library('remotes')
...
data <- y['global']
#data is the new global data set we're working with.

usdata <- data %>% filter(data$country == "United States" & data$cum_confirm>100) %>% select(time,cum_confirm)
ggplot(usdata, aes(x=time, y=cum_confirm)) + geom_line() + labs(x = "Date", y = "Cumulative Cases", title = "Total Confirmed Cases in the US Over Time")

#We can see the cases grow at an exponential rate. In this case, we can apply a simple log-linear regression to model and predict the growth. 

case <- ts(usdata$cum_confirm, start = 1, frequency = 1)
...
Attempt #2 
```{r}
hospitalization_data <- read_csv("Characteristics.csv", skip = 2)
#Trying to divide this into a test set and train set then create a model to predict hospitalization rate based on given characteristics, but I feel that the data frame needs to be restructured and cleaned up first. 
```
